...
function delegate(method, _args) {
  ...
  if (args.length === 1) {
    util.log(method.toUpperCase() + " " + callposition + util.inspect(args[0], { showHidden: true, depth: null }));
  }

  if (args.length === 2) {
    var caption = args.shift();
    util.log(method.toUpperCase() + " " + callposition +  caption + " " + util.inspect(args[0], { showHidden: true, depth: null }));
  }
