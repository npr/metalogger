var util       = require('util')
  ...
  , S3Logger   = optional('s3-streamlogger')
...
exports = module.exports = function (level) {
  if (!S3Logger) {
    throw new Error('Installation required: in order to use the s3/"sumologic" bridge, you need to: npm install s3-streamlogger. Aborting.');
  }
  ...
  if (bucketDetails.bucket && bucketDetails.access_key_id && bucketDetails.secret_access_key) {
    client = new S3Logger.S3StreamLogger(bucketDetails);
  } else {
    throw new Error('Configuration required: in order to use the s3/"sumologic" bridge, you need to: ' +
      'provide NODE_LOGGER_S3_BUCKET, NODE_LOGGER_S3_KEY_ID, and NODE_LOGGER_S3_KEY_SECRET in the process environment.');
  }
...
function delegate(method, _args) {
  ...
  if(bucketDetails.bucket.length !== 0 && bucketDetails.access_key_id.length !== 0 && bucketDetails.secret_access_key.length !== 0) {
    ...
    console.error('Configuration: Sumologic is not configured correctly, please provide `NODE_LOGGER_S3_BUCKET`, `NODE_LOGGER_S3_KEY_ID`, and `NODE_LOGGER_S3_KEY_SECRET` environment variables.');
  }
}
