...
function delegate(method, _args) {
  ...
  if(args.length === 1) {
    /* 
      If the argument is an object, then pass it through so that we can 
      search subfield values in sumologic.  If not, then it might be a formatted
      string, so we should try not to run it through util inspect.
    */
    if (args[0] !== null && typeof args[0] === 'object') {
      inspect = args[0];
    } else {
      inspect = util.inspect(args[0], { showHidden: true, depth: null }).replace(/\n/g, ' ');
    }
  }
  else if(args.length === 2) {    
    /* 
      If the second argument is an object, then pass it through so that we can 
      search subfield values in sumologic.  If not, then it might be a formatted
      string, so we should try to run it through util inspect.
    */
    if (args[0] !== null && typeof args[0] === 'object') {
      inspect = args[0];
    } else {
      inspect = util.inspect(args[0], { showHidden: true, depth: null });
    }
  }
  else if(args.length > 2) {
