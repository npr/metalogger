...
function delegate(method, _args) {
  ...
  if (args.length === 1) {
    npmlog.log(method, callposition, currDate() + util.inspect(args[0], { showHidden: true, depth: null }));
  }

  if (args.length === 2) {
    var caption = args.shift();
    npmlog.log(method, callposition + caption, currDate() + util.inspect(args[0], { showHidden: true, depth: null }));
  }
